<%@ page language="java" contentType="text/html; charset=UTF-8"
    ...
    <%
    String realFolder = ""; //웹 애플리케이션 상 절대경로
    String filename = "";
    MultipartRequest imageUp = null;
    
    String saveFolder = "imageFile"; //파일이 업로드되는 폴더
    String encType = "utf-8"; //인코딩 타입
    int maxSize = 2*1024*1024; //최대 업로드 파일 크기 5mb
    
    //현재 jsp페이지 웹 애플리케이션상의 절대경로
    ServletContext context = getServletContext();
    realFolder = context.getRealPath(saveFolder);
    
    try{
    	//전송 담당 컴포넌트를 생성하고 파일을 전송
    	//전송할 파일명 가지고 있는 객체, 서버상의 절대경로, 최대 업로드될 파일 크기, 문자코드, 기본보안 적용
    ...
    <jsp:useBean id="book" scope="page" class="shop.master.ShopBookDataBean">
    </jsp:useBean>
